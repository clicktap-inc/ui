...
| Multi-select | — | `selectionMode="multiple"` → `selectedKeys: Key[]` + `onSelectionChange(keys: Key[])`. Searchable = combobox with chips; non-searchable = button with a summary. Single-select props/types unchanged. |
...
`selectionMode="multiple"` switches the selection contract to an array: pick
several options, the menu stays open until you click away. The callback receives
a `Key[]`. It works in **both** modes — searchable and not:

```tsx
const [keys, setKeys] = useState<Key[]>([]);

// Searchable → combobox multi: type to filter, picks shown as removable chips.
<Select searchable selectionMode="multiple" selectedKeys={keys} onSelectionChange={setKeys}>
  {/* combines with <Section> groups too */}
</Select>

// Not searchable → button multi: a button whose summary lists the picks; the
// open listbox toggles each option (stays open).
<Select selectionMode="multiple" selectedKeys={keys} onSelectionChange={setKeys}>
  {/* short fixed lists */}
</Select>
```
...
- *Combobox (chips)* — `classNames.tagGroup` / `tag` / `tagRemoveButton`; chips in
  **selection order** (newest last, the usual token-input convention); full
  keyboard a11y via react-aria-components `TagGroup` (arrow-nav, Backspace/Delete).
- *Button (summary)* — comma-joined labels in **collection order** (stable, reads
  like the list), styled via `classNames.value`.
...
>
> _Caveats:_ button-mode multi has **no hidden native `<select>`** (so no
> native-form submission / autofill — consume via `selectedKeys`), and selected
> rows show via `data-[selected]` (no checkmark icon). A `tagOrder` toggle
> (collection-order chips) is deferred until a use case needs it.
...
- [ ] Multi-select (if you use `selectionMode="multiple"`): combobox shows chips (filter clears per pick, × removes); button shows a collection-order summary; menu stays open until click-away; callback gets `Key[]` (see `Multiple` / `GroupedMultiple` / `ButtonMultiple`).
...
- Stories: `libs/ui/src/components/Select/Select.stories.tsx` (`Picker`, `Searchable`, `SearchableAuto`, `AsyncDataSource`, `ServerSideSearch`, `ThemedOptions`, `Grouped`, `Multiple`, `GroupedMultiple`, `ButtonMultiple`)
